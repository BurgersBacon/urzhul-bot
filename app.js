...
const listenTwitterMentions = (db) => {
  ...
  let streamBurgrbot = Twitter.stream('statuses/filter', { track: ['@burgrbot'] });
...
const watchEnoughPendingPosts = (count) => {
...
const tweetBurgrbot = (paramsForReply, redditPost, db) => {
  TwitterLinks.post('statuses/update', paramsForReply, function(err) {
    if (redditPost) {
